...
function my_usces_delivery_after_days_script( $delivery_after_days_script){
	...
	global $wpdb;
    $wpdb -> show_errors ();
	$results = $wpdb->get_results( "SELECT disabled_date FROM Holiday ORDER BY id DESC LIMIT 1");
	$disabled_date = unserialize($results[0]->disabled_date);
	$dt_str = implode(",", $disabled_date);
...
/////////////////////// 日付を増やしたい時は最終行をコピーして最終行に追加してください。//
                if ( date_def == '2021-12-24' ){                         daystyle = 'background-color: #585858;';
                } else if ( date_def == '2021-12-25' ) {                 daystyle = 'background-color: #585858;';
                } else if ( date_def == '2021-12-26' ) {                 daystyle = 'background-color: #585858;';
                } else if ( date_def == '2022-01-01' ) {                 daystyle = 'background-color: #585858;';
                } else if ( date_def == '2022-01-02' ) {                 daystyle = 'background-color: #585858;';
                } else if ( date_def == '2022-01-03' ) {                 daystyle = 'background-color: #585858;';
//////////////////////////////////////////////////////////////////////////////////////////
                ...
                if(date_def == selected_delivery_date) {;
                  ...
                  var dt_str = '".$dt_str."';
                				if(dt_str.search(date_def)!= -1){
                							option += '<option style=\"' + daystyle + '\" value=\"' + date_def + '\" selected disabled>' + date_str + '</option>';
                							...
                							option += '<option style=\"' + daystyle + '\" value=\"' + date_def + '\" selected>' + date_str + '</option>';
												...
												var dt_str = '".$dt_str."';
                				if(dt_str.search(date_def)!= -1){
                        	option += '<option style=\"' + daystyle + '\" value=\"' + date_def + '\" disabled>' + date_str + '</option>';
                        }else{
